...
- Historical refresh from `da105477…`: the earlier tracked references exposed timer-bearing state variance and the keyboard-modal dimension mismatch. Those findings are superseded as current status but retained as causal history.
...
- Historical `da105477…` refresh: the clean-source API 36 run needed one unchanged retry and reported Statistics 2.226% and Tutorial 11.597%. The current Tutorial fix supersedes the latter value.
