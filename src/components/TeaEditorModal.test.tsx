...
describe('TeaEditorModal', () => {
    it('uses header tabs instead of the old Tea title and splits existing tea from the form', () => {
        ...
        expect(screen.getByLabelText('Search existing teas').getAttribute('autocomplete')).toBe('other');
        ...
        expect(screen.getAllByRole('combobox').every((input) => input.getAttribute('autocomplete') === 'other')).toBe(true);
    });
