...
4. For Android, run `npm run android:build:debug`, start a disposable API 24 or API 36 Pixel 2 AVD, then run `ANDROID_SERIAL=<serial> npm run visual:capture:android` and the matching `visual:compare:android:api24` or `visual:compare:android:api36` command. Capture uninstalls the app and changes emulator-wide display and animation settings, so do not use a personal or data-bearing AVD.
...
5. For API 24, check emitted syntax, required runtime polyfills, and physical fallbacks for logical CSS properties.
...
- Re-run the smallest affected target, then the full web/API24/API36 matrix.
